...
class lidar_scan_acq : public rclcpp::Node
{
    ...
    private:
        ...
        void lidar_ptCloud_pub(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
        {
            ...
            RCLCPP_INFO(this->get_logger(), "TimeStamp: '%f' - Number Meas: %u - Min Range: %f - Max Range: %f",
            		_msg->header.stamp.sec + _msg->header.stamp.nanosec/1e9, _msg->ranges.size(),
            		_msg->range_min, _msg->range_max);
